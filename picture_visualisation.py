...
def process_pixels(pixels):
    ...
    image = np.array(pixels).reshape(48, 48)  # Les images sont 48x48 pixels
...
# Ajouter une dimension supplémentaire pour les canaux de couleur (en niveaux de gris, il n'y a qu'un seul canal)
# Pour etre compatible avec le model de deep learning
...
# les e;oyions sont converties en categories numeriques\
# ex : joie → [1, 0, 0, 0, 0, 0, 0], Tristesse → [0, 1, 0, 0, 0, 0, 0], etc
labels = to_categorical(labels, num_classes=7)  # FER-2013 a 7 émotions
...
# Le programme construit un réseau de neurones convolutionnels (CNN) pour analyser les images
model = Sequential([
